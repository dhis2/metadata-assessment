...
results <- purrr::map(views$summary_uid, function(x)  getSQLView(x, d2_session = d2_session))
...
results_with_data <- results[!errored_out] %>% 
  purrr::map_df(flatten)
 
 
results_table <- results_with_data %>% 
  dplyr::left_join(views[,c("summary_uid","section","section_order","details_uid","severity")] , by =c("uid"="summary_uid")) %>% 
...
r <-httr::GET(url,content_type_json(),handle=d2_session,timeout(600)) %>% 
  httr::content(.,"text") %>% 
  jsonlite::fromJSON()
...
guidance <-  results_with_data %>% 
  dplyr::left_join(views[,c("summary_uid","section","section_order","details_uid","severity","introduction","recommendation")] , by =c("uid"="summary_uid")) %>% 
  dplyr::arrange(section,section_order) %>%
...
for (i in seq_along(sections)) {
  ...
  for (j in seq_len(NROW(this_section))) {
    ...
    cat("\n")
    cat("\n")
    cat(paste0("Issue count: ",this_section$value[j]," (",this_section$percent[j],")"))
    cat("\n")
    cat("\n")
    cat("**Introduction**")
    cat("\n")
